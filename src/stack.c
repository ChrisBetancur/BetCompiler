...
Stack* init_stack() {
    ...
    stack->top_offset = 0x10; // if value not printed it may be the offset
    return stack;
}

void stack_push(Stack* stack_frame, ASTNode* node) {
    node->offset = stack_frame->top_offset;
    stack_frame->top_offset += 0x10;
...
ASTNode* stack_pop(Stack* stack_frame) {
    ...
    stack_frame->stack_list->arr = realloc(stack_frame->stack_list->arr, stack_frame->stack_list->num_items * sizeof(struct STACK_STRUCT)); // IDK WHY IM GETTING ERROR HERE
